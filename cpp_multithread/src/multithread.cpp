...
int main(int argc, char * argv[])
{
  ...
  //多线程执行器是可以打印线程数
  RCLCPP_INFO(rclcpp::get_logger("rclcpp_11"), "cur_thread num = %d",(int)executor.get_number_of_threads());
  //std::cout<<"cur_thread num = "<<executor.get_number_of_threads()<<std::endl;
  executor.spin();
  rclcpp::shutdown();
  return 0;
}